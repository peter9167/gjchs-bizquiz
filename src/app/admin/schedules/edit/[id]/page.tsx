...
import { Question, QuizSchedule } from '@/types/database'

export default function EditSchedule({ params }: { params: { id: string } }) {
  const [questions, setQuestions] = useState<Question[]>([])
  const [schedule, setSchedule] = useState<QuizSchedule | null>(null)
  ...
  const loadScheduleAndQuestions = async () => {
    try {
      // 기존 스케줄 데이터 로드
      ...
      setFormData({
        ...
        question_ids: scheduleData.question_ids || [],
        ...
        time_limit_minutes: scheduleData.time_limit_minutes,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedSchedule = await AdminManager.updateSchedule(params.id, formData)
      if (updatedSchedule) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">일정 수정</h1>
            <p className="text-gray-600">기존 퀴즈 일정을 수정합니다</p>
          </div>
